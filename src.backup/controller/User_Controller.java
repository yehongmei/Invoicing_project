...
public class User_Controller {
    private static final int IMG_HEIGHT = 40;// 验证码图片高�?
    private static final int IMG_WIDTH = 120;// 验证码图片宽�?
  
    @RequestMapping("identifyCode")
    @ResponseBody
    public boolean identifyCode(String user_code,HttpSession session){
    	return session.getAttribute("identifyCode").equals(user_code);
    }
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        // 6.写在图片上随机数
        // String random = drawRandomNum((Graphics2D) g,"ch");//生成中文验证码图�?
        // String random = drawRandomNum((Graphics2D) g,"nl");//生成数字和字母组合的验证码图�?
        // String random = drawRandomNum((Graphics2D) g,"n");//生成纯数字的验证码图�?
        // String random = drawRandomNum((Graphics2D) g,"l");//生成纯字母的验证码图�?
        String random = drawRandomNum((Graphics2D) graphics, "nl");// 根据客户端传递的createTypeFlag标识生成验证码图�?
        // 7.将随机数存在session�?
        request.getSession().setAttribute("identifyCode", random);
        // 8.设置响应头�?知浏览器以图片的形式打开
     ...
     *         Java1.5增加了新特�?：可变参数：适用于参数个数不确定，类型确定的情况
     *         ，java把可变参数当做数组处理�?注意：可变参数必须位于最后一�?
     */
